...
public class meanData_Plotter {
	...
	public meanData_Plotter(String title, String xAxis) {
        super();      
        String yAxis = title;
    ...
    public XYSeriesCollection getXYSeries(int node) {
  	...
  	public static void main() throws InterruptedException, IOException {
         ...
         meanData_Plotter mcf = new meanData_Plotter("Node Current [mA]", "Volt [mV]");
        JFrame frame = new JFrame("Standard Deviation of Node Data");
	        ...
	        // generating the data needed for plotting/visualizing
	  		...
	  		for (int index = start; index < end; index++){
	  			...
	  			for (int i = 0; i < length ; i++){
	  				xvalue = data_vault.data_x[i][1]; // data is created for the x-axis
	  		        //System.out.println(yvalue + " -- y -- ");
		  			double[][] data = data_vault.fetch_data(start, end, node, length); 
	  		        yvalue = data_manipulate.devi_data[i][node];
	  		        //yvalue = data_vault.x_value;
	  		        //System.out.println("(x,y)  value " + " (" + xvalue + "," + yvalue + ") " + " of node " + node);
	  				//System.out.println(" node  -- " + node);//devi_data[i][node]);
